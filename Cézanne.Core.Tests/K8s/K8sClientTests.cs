...
namespace Cézanne.Core.Tests.K8s
{
    public class K8SClientTests : ITempFolder
    {
        ...
        public async Task CallApi()
        {
            ...
            {
                using K8SClient client = new(new K8SClientConfiguration { Base = mockServer.Urls.First() },
        ...
        public void YamlKubeConfig()
        {
            ...
            File.WriteAllText(
                ...
                """);
            using K8SClient client = new(new K8SClientConfiguration { Kubeconfig = kubeConfig },
                new NullLoggerFactory());
            Assert.Multiple(() =>
            {
