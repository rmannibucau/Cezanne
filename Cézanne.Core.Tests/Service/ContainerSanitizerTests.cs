...
namespace Cézanne.Core.Tests.Service
{
    public class ContainerSanitizerTests
    {
        ...
        private readonly ContainerSanitizer _sanitizer =
            new(new Logger<ContainerSanitizer>(new NullLoggerFactory()));
        ...
        private void _AssertSanitization(string kind, string input, string output)
        {
            Assert.That(
                JsonSerializer.Serialize(
                    _sanitizer.DropCpuResources(
                        kind,
                        JsonSerializer.Deserialize<JsonObject>(input, Options)!
                    ),
                    Options
                ),
                Is.EqualTo(output)
