...
{
    private readonly ConditionJsonEvaluator _conditionJsonEvaluator = new ConditionJsonEvaluator(new Logger<ConditionJsonEvaluator>(new NullLoggerFactory()));

    [TestCaseSource(nameof(DataSet))]
    public bool Eval(string condition, string input)
    {
        var cond = JsonSerializer.Deserialize<Manifest.AwaitCondition>(condition, Jsons.Options) ?? throw new ArgumentNullException(nameof(condition));
