...
namespace Cézanne.Core.Maven
{
    public class MavenService : IDisposable
    {
        ...
        public MavenService(MavenConfiguration configuration, ILoggerFactory loggerFactory)
        {
            _logger = loggerFactory.CreateLogger<MavenService>();
        ...
        private async Task<string> _DoFind(string raw)
        {
            ...
            if (segments.Length < 3)
            {
                ...
                throw new ArgumentException($"Invalid path: {raw}");
            }
