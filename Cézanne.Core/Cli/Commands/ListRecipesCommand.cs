...
{
    public class ListRecipesCommand(
        ILogger<ListRecipesCommand> logger,
        RecipeHandler recipeHandler) : AsyncCommand<ListRecipesCommand.Settings>
    {
        public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
        {
            ...
            var manifest = await AnsiConsole.Progress()
                ...
                {
                    logger.LogTrace("Looking up recipe id='{Id}' location='{Location}' manifest='{Manifest}'", id,
                        settings.From,
                        settings.Manifest);
                    return await recipeHandler.FindManifest(settings.From, settings.Manifest, id,
                               new ProgressHandler(ctx).OnProgress) ??
                           throw new InvalidOperationException("Didn't find the requested manifest");
                });
        ...
        {
            return JsonSerializer.Serialize(new { Items = manifest.Recipes.Select(it => it.Name).ToList() },
                Jsons.Options);
        }
        ...
        private string _AsText(Manifest manifest)
        {
            ...
            return "Found recipes:\n" + string.Join('\n', manifest!.Recipes
                .OrderBy(it => it.Name)
                .Select(it => $"- {it.Name}" + (it.Description is not null ? $": {it.Description}" : "")));
        }
