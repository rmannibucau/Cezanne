...
namespace Cézanne.Core.K8s
{
    public class K8SClient : IAsyncDisposable, IDisposable
    {
        ...
        public K8SClient(
            K8SClientConfiguration configuration,
        ...
        )
        {
            ...
            _droppedAttributes = (configuration.ImplicitlyDroppedAttributes ?? []).Select(
                path => new JsonPatch(PatchOperation.Remove(JsonPointer.Parse(path)))
            );
            ...
            {
                DefaultNamespace = File.ReadAllText(
            ...
            if (kubeconfig != null && kubeconfig != "skip")
            {
                ...
                {
                    baseUrl =
                        _InitFromKubeConfig(HttpMessageHandler, HttpClient.DefaultRequestHeaders)
                        ?? baseUrl;
                }
            }
            ...
            if (
                configuration.PrivateKey is not null
                && configuration.PrivateKeyCertificate is not null
            )
            {
                _logger.LogDebug(
                    "Loading certificate '{Certificate}' with private key '{PrivateKey}'",
            ...
            HttpClient.BaseAddress = new Uri(
                baseUrl ?? throw new ArgumentException("No base url found")
            );
        ...
        )
        {
            ...
            if (_dryRun)
            {
                ...
                response = await Task.FromResult(
            ...
            if (_verbose)
            {
                ...
                _logger.LogInformation(
                    ...
                    method,
                    relativeUri,
                    _FormatHeaders(message.Headers),
        ...
        public async Task<string> ToBaseUri(JsonObject prepared)
        {
            ...
            {
                specPrefix = (specPrefix.StartsWith('/') ? specPrefix[1..] : specPrefix).Replace(
                    "${namespace}",
                    namespaceValue,
            ...
            var nsSegment = _IsSkipNameSpace(kindLowerCased)
                ? ""
        ...
        public async Task<IEnumerable<T>> ForDescriptor<T>(
                ...
                switch (json?.GetValueKind())
                {
                    ...
                    default:
                        throw new InvalidOperationException(
                            $"Invalid descriptor {descriptorContent}"
                        );
                }
            }
            catch (Exception e)
        ...
        )
        {
            ...
            var context =
                KubeConfig.Contexts.FirstOrDefault(
                    it => it?.Name == KubeConfig.CurrentContext,
                    null
            ...
            var user = KubeConfig.Users.First(it => it.Name == context.Context.User);
            var cluster = KubeConfig.Clusters.First(it => it.Name == context.Context.Cluster);
                ...
                )
                {
                    lock (httpMessageHandler)
                    {
                        if (
                            !File.Exists(tokenFile)
                            || (
                                requestHeaders.Authorization is not null
        ...
        )
        {
            if (cluster.CertificateAuthorityData is not null)
            {
                ...
                && File.Exists(cluster.CertificateAuthority)
            )
            {
        ...
        private KubeConfig _LoadKubeConfigFromYaml(FileStream stream)
        {
            ...
            if (obj.TryGetValue("clusters", out var clusters))
            {
                config.Clusters = (clusters as IEnumerable<object> ?? [])
                    ...
                    {
                        var clusterData =
                            it["cluster"] as IDictionary<string, object>
                        ...
                        return new KubeConfig.NamedCluster
                        {
                            ...
                            {
                                Server = clusterData.TryGetValue("server", out var server)
                                    ? server as string
                                    : Base,
                                InsecureSkipTlsVerify =
                                    clusterData.TryGetValue(
                                        "insecure-skip-tls-verify",
                                        out var istv
                                    ) && bool.Parse(istv as string ?? "false"),
                                CertificateAuthorityData = clusterData.TryGetValue(
                                    "certificate-authority-data",
                                    out var cad
                                )
                                    ? cad as string
                                    : null,
                                CertificateAuthority = clusterData.TryGetValue(
            ...
            if (obj.TryGetValue("users", out var users))
            {
                config.Users = (users as IEnumerable<object> ?? [])
                    ...
                    {
                        var user =
                            it["user"] as IDictionary<string, object>
                            ?? ImmutableDictionary<string, object>.Empty;
                        return new KubeConfig.NamedUser
                        {
                            ...
                            {
                                Token = user.TryGetValue("token", out var token)
                                    ? token as string
                                    : null,
                                TokenFile = user.TryGetValue("token", out var tokenFile)
                                    ? tokenFile as string
                                    : null,
                                ClientCertificateData = user.TryGetValue(
                                    "client-certificate-data",
                                    out var ccd
                                )
                                    ? ccd as string
                                    : null,
                                ClientKeyData = user.TryGetValue("client-key-data", out var ckd)
                                    ? ckd as string
                                    : null,
                                ClientCertificate = user.TryGetValue(
                                    "client-certificate",
                                    out var clientCertificate
                                )
                                    ? clientCertificate as string
                                    : null,
                                ClientKey = user.TryGetValue("client-key", out var clientKey)
                                    ? clientKey as string
                                    : null,
                                Username = user.TryGetValue("username", out var username)
                                    ? username as string
                                    : null,
            ...
            if (obj.TryGetValue("contexts", out var contexts))
            {
                config.Contexts = (contexts as IEnumerable<object> ?? [])
                    ...
                    {
                        var contextData =
                            it["context"] as IDictionary<string, object>
                            ?? ImmutableDictionary<string, object>.Empty;
