...
using Cezanne.Core.Cli.Progress;
...
namespace Cezanne.Core.Cli.Command
{
    ...
    ) : AsyncCommand<ApplyCommand.Settings>, ICompletable
    {
        public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
        {
            ...
            var tasksToInstall = await AnsiConsole
                .Progress()
                .AutoClear(true)
                .HideCompleted(false)
                ...
                {
                    logger.LogTrace(
                        "Applying id='{Id}' location='{Location}' manifest='{Manifest}'",
                        id,
                        settings.From,
                        settings.Manifest
                    );
            ...
            await AnsiConsole
                ...
                .StartAsync(
                    $"Applying id='{id}' location='{settings.From}' manifest='{settings.Manifest}'",
                    async ctx =>
                    {
        ...
        private async Task<bool> _DoApply(
            JsonObject prepared,
            string kind,
            int retries,
            bool dryRun,
            string fieldValidation,
            ISet<string> statefulSetsAllowedSpecAttributes
        )
        {
            ...
            logger.LogInformation(
                "Applying {kind} '{Name}' on namespace '{Namespace}'",
                name,
                ns,
                kind[..^1]
            );
            ...
            if (findResponse.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (kind != "persistentvolumeclaims")
                {
                    ...
                    if (
                        payload["metadata"]!
                            .AsObject()
                            .TryGetPropertyValue("resourceVersion", out var crv)
                        && crv is null
                        && metadata.TryGetPropertyValue("resourceVersion", out var rs)
                        && recipeHandler is not null
                    )
                    {
                        payload = new JsonPatch(
                            PatchOperation.Add(JsonPointer.Parse("/metadata/resourceVersion"), rs)
                        )
                            .Apply(payload)
                            .Result!.AsObject();
                    }
                    ...
                    prepared["metadata"]!
                        .AsObject()
                        .TryGetPropertyValue("annotations", out var annotations);
                    if (annotations is not null)
                    {
                        ...
                        )
                        {
                            using var deleteResponse = await client.SendAsync(
                                HttpMethod.Delete,
                                completeUri + "?gracePeriodSeconds=-1",
                            ...
                            else
                            {
                                while (true)
                                {
                                    ...
                                    logger.LogTrace(
                                        "{Namespace}/{Name} deletion:{Response}",
                                        ns ?? "-",
                                        name,
                                        response
                                    );
                        ...
                        if (
                            annotations
                                .AsObject()
                                .TryGetPropertyValue(
                                    "io.yupiik.bundlebee/putOnUpdate",
                                    out var update
                                )
                        ...
                        )
                        {
                            ...
                            return putResponse.StatusCode switch
                            {
                                ...
                                _
                                    => throw new InvalidOperationException(
                                        $"Can't put {completeUri}: {putResponse}"
                                    )
                            };
                        }
                    }
                    ...
                    var patchType =
                        ...
                        && annotations
                            .AsObject()
                            .TryGetPropertyValue(
                                "io.yupiik.bundlebee/patchContentType",
                                out var patchContentType
                            )
                    ...
                    using (patchResponse)
                    {
                        return patchResponse.StatusCode switch
                        {
                            ...
                            _
                                => throw new InvalidOperationException(
                                    $"Can't patch {completeUri}: {patchResponse}"
                                )
                        };
                    }
                }
            ...
            if (postResponse.StatusCode == HttpStatusCode.Conflict && retries > 0)
            {
                ...
                if (
                    ...
                    && obj!.TryGetPropertyValue("details", out var details)
                    && details is not null
        ...
        public class Settings : CommandSettings
        {
            ...
            [Description(
                "List of fields allowed for `StatefulSet` updates - all are not updatable."
            )]
