...
namespace Cezanne.Core.Interpolation
{
    ...
    public sealed class Substitutor
    {
        ...
        [ThreadStatic]
        private static IDictionary<string, string> contextualPlaceholders = ImmutableDictionary<
            string,
            string
        >.Empty;
        ...
        public readonly ConcurrentDictionary<
            string,
            IList<Action<string, string?, string?>>
        > LookupListeners = new();
        ...
        )
        {
            ...
            _Lookup = (id, key, defaultValue) =>
            {
                ...
                var listeners =
                    id is not null && LookupListeners.TryGetValue(id, out var ls) ? ls : [];
        ...
        public Substitutor(Func<string, string?> lookup, K8SClient? k8s, MavenService? maven)
            : this((key, _) => lookup(key), k8s, maven) { }
        ...
        private string? _DoReplace(
            Manifest.Recipe? alveolus,
            LoadedDescriptor? desc,
            string? source,
            string? id
        )
        ...
        )
        {
            ...
            if (sep > 0)
            {
                ...
                return startOfString
                    + _DoGetOrDefault(alveolus, descriptor, actualKey, fallback, id)
                    + endOfString;
            }

            return startOfString
                + _DoGetOrDefault(alveolus, descriptor, key, null, id)
                + endOfString;
        }
        ...
        private string _DoGetOrDefault(
            Manifest.Recipe? alveolus,
            LoadedDescriptor? descriptor,
            string varName,
        ...
        private string? _DefaultLookups(
            Manifest.Recipe? recipe,
            LoadedDescriptor? desc,
            string varName,
            string? id
        ...
        private string? _DefaultLookupsWithParameters(
            Manifest.Recipe? recipe,
            LoadedDescriptor? desc,
            string placeholder,
            string? id
        )
        {
            if (placeholder.StartsWith("bundlebee-directory-json-key-value-pairs-content:"))
            {
                var result = Replace(
                        ...
                        "{{bundlebee-directory-json-key-value-pairs:"
                            + placeholder[
                                "bundlebee-directory-json-key-value-pairs-content:".Length..
                            ]
                            + "}}",
                        id
                    )
            ...
            if (placeholder.StartsWith("bundlebee-directory-json-key-value-pairs:"))
            {
                ...
                var files = (
                    lastSep < 0
                        ...
                        : new DirectoryInfo(directory[..lastSep])
                ).GetFiles(
                    lastSep < 0 ? "*" : directory[(lastSep + 1)..],
            ...
            if (placeholder.StartsWith("bundlebee-base64:"))
            {
                return Convert.ToBase64String(
                    Encoding.UTF8.GetBytes(placeholder["bundlebee-base64:".Length..])
                );
            }
            ...
            if (placeholder.StartsWith("bundlebee-decipher:"))
            {
                ...
                if (sep < 0)
                {
                    throw new ArgumentException(
                        "Usage: {{bundlebee-decipher:$masterKeyPlaceholder,$cipheredValue}}"
                    );
                }

                return new SimpleCodec(confAndValue[..sep]).Decipher(
                    confAndValue[(sep + 1)..].Trim()
                );
            }
            ...
            if (placeholder.StartsWith("kubeconfig.cluster.") && placeholder.EndsWith(".ip"))
            {
                ...
                var name = placeholder[
                    "kubeconfig.cluster.".Length..(placeholder.Length - ".ip".Length)
                ];
                var url =
                    _K8s.KubeConfig == null
                        ? _K8s.Base
                        : _K8s
                            .KubeConfig.Clusters?.FirstOrDefault(
                                it => it.Name == name,
                                new KubeConfig.NamedCluster
                                {
                                    Cluster = new KubeConfig.Cluster { Server = _K8s.Base }
                                }
                            )
                            .Cluster?.Server;
            ...
            {
                return _Maven
                    ?.FindMavenServer(placeholder["bundlebee-maven-server-username:".Length..])
                    ?.Username;
            }
            ...
            {
                return _Maven
                    ?.FindMavenServer(placeholder["bundlebee-maven-server-password:".Length..])
                    ?.Password;
            }
        ...
        private string? _FindKubernetesValue(string key, string sep)
        {
            ...
            if (
                segments.Length >= 8
                && segments.Length <= 10
                && segments is ["kubernetes", _, "serviceaccount", _, "secrets", _, "data", ..]
            )
            {
        ...
        )
        {
            ...
            do
            {
                ...
                using var result = _K8s
                    ?.SendAsync(
                        HttpMethod.Get,
                        $"/api/v1/namespaces/{namespaceValue}/serviceaccounts/{account}"
                    )
                    .GetAwaiter()
                    .GetResult();
                ...
                {
                    var selectedSecret = objs.First(it =>
                        it["name"]?.ToString().StartsWith(secretPrefix) ?? false
                    );
                    using var secretResponse = _K8s
                        ?.SendAsync(
                            HttpMethod.Get,
