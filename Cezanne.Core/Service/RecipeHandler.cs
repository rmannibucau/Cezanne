...
namespace Cezanne.Core.Service
{
    ...
    {
        public async Task ExecuteOnceOnRecipe(
            string prefixOnVisitLog,
            Manifest manifest,
            ...
            string alreadyHandledMarker,
            string id,
            Action<string, double> progress
        )
        {
            HashSet<string> alreadyDone = [];
            await ExecuteOnRecipe(
                prefixOnVisitLog,
                manifest,
                recipe,
                userCallback,
        ...
        public async Task ExecuteOnRecipe(
            string prefixOnVisitLog,
            Manifest manifest,
            Manifest.Recipe recipe,
            Func<VisitContext, Task>? userCallback,
        ...
        )
        {
            ...
            Func<VisitContext, Task> combinedCallbacks =
                userCallback == null
                    ? internalCallback
                    : async ctx =>
                    {
        ...
        private async Task _OnRecipe(
            string prefixOnVisitLog,
            Manifest manifest,
            Manifest.Recipe from,
        ...
        )
        {
            ...
            IEnumerable<Manifest.DescriptorRef> currentExcludes =
            [
                .. excludes,
                .. from.ExcludedDescriptors ?? []
            ];
            var currentPatches = !(from.Patches ?? []).Any()
                ? patches
                : _MergePatches(patches, from.Patches);
            var currentPlaceholders =
                from.Placeholders is null || !from.Placeholders.Any()
                    ? placeholders
                    : placeholders
        ...
        private async Task _AfterDependencies(
            Manifest manifest,
            Manifest.Recipe from,
            IDictionary<Predicate<string>, Manifest.Patch> patches,
            IEnumerable<Manifest.DescriptorRef> excludes,
        ...
        private async Task _PrepareDescriptors(
            Manifest manifest,
            Manifest.Recipe from,
            IDictionary<Predicate<string>, Manifest.Patch> patches,
            IDictionary<string, string> placeholders,
            ...
            Func<VisitContext, LoadedDescriptor, Task> onDescriptor,
            IDictionary<Predicate<string>, Manifest.Patch> currentPatches,
            IEnumerable<LoadedDescriptor> descs,
            string? id
        )
        {
            var tasks = descs
        ...
        private LoadedDescriptor _Prepare(
            Manifest.Recipe from,
        ...
        {
            return substitutor.WithContext(
                placeholders,
                () =>
                {
                    var content = desc.Content;
                    var patches = currentPatches.Where(it =>
                        it.Key(desc.Configuration.Name ?? "")
                        || it.Key($"{desc.Configuration.Name}.{desc.Extension}")
                    );
        ...
        private IEnumerable<IEnumerable<LoadedDescriptor>> _RankDescriptors(
            IEnumerable<LoadedDescriptor> descriptors
        )
        {
            List<IEnumerable<LoadedDescriptor>> rankedDescriptors =
                new(
                    1 /*generally 1 or 2*/
                );
        ...
        private async Task<LoadedDescriptor> _FindDescriptor(
            Manifest.Descriptor desc,
            ArchiveReader.Cache cache,
            string? id,
            Action<string, double>? progress
        )
        {
                    ...
                    + _FindExtension(
                        desc.Name
                            ?? throw new ArgumentNullException(
                                $"Descriptor name can't be null {desc}",
                                nameof(desc.Name)
                            ),
                        type
                    )
            ...
            var archive = await cache.LoadArchive(
                desc.Location,
                id,
                new ActionProgress(desc.Location, progress)
            );
            var content = archive.Descriptors.TryGetValue(resource, out var descriptors)
                ? descriptors
                : throw new InvalidOperationException(
                    $"No descriptor '{resource}' found in '{desc.Location}'"
                );
        ...
        private IEnumerable<Manifest.Descriptor> _SelectDescriptors(
            Manifest.Recipe from,
            IEnumerable<Manifest.DescriptorRef> excludes
        )
        {
            return (from.Descriptors ?? []).Where(desc =>
                conditionEvaluator.Test(desc.IncludeIf)
                && !excludes.Any(it =>
        ...
        private Func<Task> _PrepareDependencyResolution(
            Manifest manifest,
            Manifest.Dependency it,
            ArchiveReader.Cache cache,
            string? id,
            IEnumerable<Manifest.DescriptorRef> currentExcludes,
            IDictionary<Predicate<string>, Manifest.Patch> currentPatches,
            IDictionary<string, string> currentPlaceholders,
            Func<VisitContext, Task> onRecipe,
            Action<string, double>? progress
        )
        {
            ...
            var name =
                it.Name ?? throw new ArgumentNullException(nameof(it.Name), "No dependency name");
        ...
        private async Task<RecipeContext> _FindRecipe(
            string from,
            string recipeName,
            ArchiveReader.Cache cache,
            string? id,
        ...
        private IDictionary<Predicate<string>, Manifest.Patch> _MergePatches(
            IDictionary<Predicate<string>, Manifest.Patch> current,
            IEnumerable<Manifest.Patch>? others
        )
        {
        ...
        public async Task<Manifest?> FindManifest(
            string? from,
            string? manifest,
            string? id,
            Action<string, double>? progress
        )
        {
            ...
            var archive = await archiveReader
                .NewCache()
                .LoadArchive(
                    from,
                    id,
                    new ActionProgress(from ?? manifest ?? "manifest", progress)
                );
            return archive.Manifest;
        }

        public async Task<IEnumerable<RecipeContext>> FindRootRecipes(
            string? from,
            string? manifest,
            string? recipe,
            string? id,
        ...
        public async Task<IEnumerable<RecipeContext>> _DoFindRootRecipes(
            string? from,
            string? manifest,
            string? recipe,
            string? id,
        ...
        )
        {
            if (manifest is not null && manifest != "skip")
            {
                ...
                var recipeName =
                    recipe == "auto" && mf.Recipes.Count() == 1 ? mf.Recipes.First().Name : recipe;
                var selected = mf.Recipes.Where(it => it.Name == recipeName).First();
                return await Task.FromResult<IEnumerable<RecipeContext>>(
                    [new RecipeContext(mf, selected)]
                );
            }
            ...
            var selectedRecipe =
                recipe == "auto" && archive.Manifest.Recipes.Count() == 1
        ...
        public Manifest ReadManifest(string manifest, string? id)
        {
                ...
                return manifestReader.ReadManifest(
                    ...
                    s =>
                        throw new ArgumentException(
                            "References unsupported in manifest memory mode",
                            nameof(s)
                        ),
                    id
                );
            ...
            var bundlebee =
                Directory.GetParent(Path.GetFullPath(manifest))?.FullName
                ?? throw new ArgumentException(
                    $"Invalid manifest path {manifest}",
                    nameof(manifest)
                );
                    ...
                    new()
                    {
                        ...
                        [
                            ...
                            .. excludedDescriptors is null or "none"
                                ...
                                : excludedDescriptors
                                    .Split(',')
                                    .Select(it => new Manifest.DescriptorRef
                                    {
                                        Name = it,
                                        Location = "*"
                                    }),
