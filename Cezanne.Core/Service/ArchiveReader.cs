...
namespace Cezanne.Core.Service
{
    ...
    )
    {
        ...
        public Archive Read(string coords, string path, string? id)
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                if (File.Exists(folderManifest))
                {
                    ...
                    var descriptors = Directory
                        .EnumerateFiles(
                            Path.Combine(path, "bundlebee/kubernetes"),
                            "*.*",
            ...
            var manifest =
                zip.GetEntry("bundlebee/manifest.json")
            ...
            var zipDescriptors = zip
                .Entries.Where(entry =>
                    entry.FullName.StartsWith("bundlebee/kubernetes/")
        ...
        public class Cache(ArchiveReader archiveReader)
        {
            ...
            public Task<Archive> LoadArchive(
                string coord,
                string? id,
                IProgress<double>? onProgress
            )
            {
                return _archives.AddOrUpdate(
                    coord,
                    key => _DoLoadArchive(key, id, onProgress),
                    (_, old) => old
                );
            }

            private async Task<Archive> _DoLoadArchive(
                string coord,
                string? id,
                IProgress<double>? onProgress
            )
