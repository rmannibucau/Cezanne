...
namespace Cezanne.Core.Service
{
    ...
    )
    {
        public async Task Await(string command, LoadedDescriptor descriptor, long timeout)
        {
            ...
            var conditions = (descriptor.Configuration.AwaitConditions ?? []).Where(it =>
                it.Command == command
            );
            ...
            await Asyncs.All(
                false,
                conditions.Select<Manifest.AwaitConditions, Func<Task>>(it =>
                {
                    async Task awaiter()
                    {
        ...
        private async Task _Exists(LoadedDescriptor descriptor, DateTime timeout, bool expected)
        {
            ...
            {
                var result = await client.ForDescriptor(
                    descriptor.Content,
        ...
        private async Task _Await(
            Manifest.AwaitConditions conditions,
            DateTime expiration,
        ...
        )
        {
            ...
            {
                Func<IEnumerable<Task>, Task> combiner =
                    conditions.OperatorType == Manifest.ConditionOperator.Any
                        ? Task.WhenAny
                        : Task.WhenAll;
                await combiner(
                    conditions.Conditions.Select(it => _Await(it, expiration, descriptor))
                );
            }
        }

        private async Task _Await(
            Manifest.AwaitCondition condition,
            DateTime expiration,
            LoadedDescriptor descriptor
        )
        {
        ...
        private async Task<bool> _Evaluate(
            Manifest.AwaitCondition condition,
