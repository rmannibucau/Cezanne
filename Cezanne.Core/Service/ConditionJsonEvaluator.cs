        ...
        public bool Evaluate(Manifest.AwaitCondition condition, JsonElement body)
        {
            switch (condition.TypeValue)
            {
                case Manifest.AwaitConditionType.JsonPointer:
                    ...
                    {
                        logger.LogDebug(
                            "No result for {condition} from JSON={body} and evaluation={json}",
                            condition,
                            body,
                            json
                        );
                    ...
                    var result = _Evaluate(
                        condition.OperatorType ?? Manifest.JsonPointerOperator.EqualsValue,
                        condition.Value?.ToString() ?? "",
                        evaluated
                    );
                    logger.LogDebug(
                        "{condition}={result} from JSON={body} and evaluation={json}",
                        condition,
                        result,
                        body,
                        json
                ...
                case Manifest.AwaitConditionType.StatusCondition:
                    ...
                    if (
                        conditions is null
                        || !conditions.HasValue
                        || conditions.Value.ValueKind != JsonValueKind.Array
                    )
                    {
                ...
                default:
                    throw new ArgumentException(
                        $"Unsupported value '{condition.TypeValue}'",
                        nameof(condition)
                    );
            }
        }
        ...
        private bool _Evaluate(Manifest.JsonPointerOperator type, string value, string evaluated)
        {
            return type switch
            {
                ...
                Manifest.JsonPointerOperator.EqualsIgnoreCase
                    => string.Equals(evaluated, value, StringComparison.OrdinalIgnoreCase),
                ...
                _
                    => throw new ArgumentException(
                        $"Unsupported comparison type: {type}",
                        nameof(type)
                    )
            };
        }
