            ...
            await using (mockServer)
            {
                await using K8SClient client =
                    new(
                        ...
                        {
                            Base = mockServer.Urls.First(),
                            Kubeconfig = "skip"
                        },
                        new Logger<K8SClient>(loggerFactory),
                        new Logger<ApiPreloader>(loggerFactory)
                    );
                ...
                Assert.Multiple(async () =>
                {
                    Assert.That(
                        ...
                        Is.EqualTo(
                            new List<string>
                            {
                                $"[Cezanne.Core.K8s.K8SClient][Debug] Using base url 'http://127.0.0.1:{new Uri(mockServer.Urls.First()).Port}'",
                                "[Cezanne.Core.K8s.K8SClient][Information] Execution will use dry-run mode",
                                "[Cezanne.Core.K8s.K8SClient][Information] GET /foo\n\nHTTP/1.1 200 OK\nx-dry-run: true\n\n{}"
                            }
                        )
                    );
        ...
        public async Task YamlKubeConfig()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(
                    ...
                    Is.EqualTo(
                        new Uri(client.Base ?? throw new ArgumentException("no client.Base"))
                    )
                );
